...
class Driver : public rclcpp::Node, public CallbackHandler
{
  ...
  using TimeSurfaceMsg = libcaer_driver_eventframe_msgs::msg::TimeSurface;
...
private:
  ...
  // ---------- /events_rep (time surface) state ----------
  void computeAndPublishTimeSurface(uint64_t headerStampNs);
  void tsTimerCallback();
  rclcpp::Publisher<TimeSurfaceMsg>::SharedPtr repPub_;
  rclcpp::TimerBase::SharedPtr tsTimer_;
  // Accumulation buffer of raw events (sensor microseconds). SoA layout.
  // Protected by tsBufMutex_ when tsTimerDriven_ is true.
  std::mutex tsBufMutex_;
  std::vector<uint16_t> evBufX_;
  std::vector<uint16_t> evBufY_;
  std::vector<uint8_t>  evBufP_;     // 0=OFF, 1=ON
  std::vector<int64_t>  evBufTus_;   // sensor timestamp in microseconds
  int64_t evBufStartTus_{-1};        // -1 = empty
  // Scratch buffers used by tsTimerCallback to drain accumulators under lock.
  std::vector<uint16_t> evDrainX_;
  std::vector<uint16_t> evDrainY_;
  std::vector<uint8_t>  evDrainP_;
  std::vector<int64_t>  evDrainTus_;
  // Pre-allocated output (size = channels * height * width). Reused per pub.
  std::vector<float>    tsData_;
  // ---- params -----
  uint32_t tsWindowUs_{3000};        // temporal window in microseconds (default 3 ms)
  uint32_t tsNumBins_{5};            // number of temporal bins → channels = 2 * n_bins
  uint32_t tsChannels_{10};          // 2 * tsNumBins_
  bool     tsEnabled_{true};
  bool     tsTimerDriven_{true};     // if true: publish at fixed rate even if 0 events
  // ------------------------------------------------------
