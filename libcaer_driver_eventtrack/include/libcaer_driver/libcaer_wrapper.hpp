...
public:
  ...
  // Override libcaer's default packet container flush interval (10 ms).
  // Must be called after initialize() (so device_ is open) and before
  // startSensor(). Values are in microseconds; minimum 1 us, sensible
  // range 1000–10000 us for high-FPS event-rep pipelines.
  void setPacketContainerInterval(uint32_t microseconds) {
    if (device_) device_->setPacketContainerInterval(microseconds);
  }
