# libcaer_driver_eventframe — 개조 컨텍스트 요약
...
| `libcaer_driver_eventframe` | 드라이버 노드 (개조 핵심) |
...
colcon build --symlink-install --packages-select \
  libcaer_driver_eventframe_msgs libcaer_driver_eventframe
source install/setup.bash

# 드라이버 실행 (파라미터 오버라이드 예시)
ros2 launch libcaer_driver_eventframe driver_node.launch.py \
...
# 시각화 노드 (5fps)
ros2 launch libcaer_driver_eventframe timesurface_vis.launch.py
